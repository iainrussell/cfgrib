...
def test_translator(dataarray1):
    res = cfcoords.translator('level', 'hPa', lambda x: False, 'level', dataarray1)
    assert dataarray1.equals(res)

    with pytest.raises(ValueError):
        cfcoords.translator('level', 'hPa', lambda x: True, 'level', dataarray1)

    res = cfcoords.translator('level', 'hPa', cfcoords.is_vertical_pressure, 'level', dataarray1)
    assert dataarray1.equals(res)

    with pytest.raises(ValueError):
        cfcoords.translator('level', 'hPa', cfcoords.is_latitude, 'level', dataarray1)

    res = cfcoords.translator('level', 'Pa', cfcoords.is_vertical_pressure, 'level', dataarray1)
    assert not dataarray1.equals(res)

    res = cfcoords.translator('step', 'h', cfcoords.is_forecast_period, 'step', dataarray1)
    assert dataarray1.equals(res)


def test_translate(dataarray1, dataarray2):
    result = cfcoords.translate(dataarray1)

    assert 'latitude' in result.coords
    assert 'longitude' in result.coords
    assert 'time' in result.coords

    result = cfcoords.translate(dataarray2)
